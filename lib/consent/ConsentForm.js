...
class ConsentForm {
  static strategies = {};
  static customUI = null; // Holds the custom UI handler
  /**
   * Register a custom UI handler for the consent form.
   * @param {Function} fn - The custom UI function (should accept onAccept, onReject)
   */
  static setCustomUI(fn) {
    if (typeof fn !== 'function') {
      throw new ValidationError('[ConsentForm.setCustomUI] fn must be a function');
    }
    ConsentForm.customUI = fn;
  }
  constructor(options = {}) {
  ...
  showConsentForm(onAccept, onReject) {
    ...
    // Use custom UI if registered
    if (ConsentForm.customUI) {
      return ConsentForm.customUI(onAccept, onReject);
    }
